...
// src/services/panicEventsmix.ts
var MIX_BASE = "https://integrate.za.mixtelematics.com";
var MIX_USERNAME = "staplepointbi@gmail.com";
var MIX_PASSWORD = "Bplsupport1234?";
var MIX_CLIENT_ID = "integrate.za.ch";
var MIX_CLIENT_SECRET = "yPNIMg2B14O5di7x";
var cachedToken = null;
var tokenExpiry = 0;
async function getMixToken() {
  ...
  try {
    ...
    tokenExpiry = Math.floor(Date.now() / 1e3) + (data.expires_in || 3600);
    return cachedToken;
  } catch (err) {
...
async function fetchAndSavePanicEvents(clientId) {
  try {
    ...
    for (const ev of events || []) {
      ...
      await addDoc(collection(db, "panicEvents"), {
        eventId,
        vehicleId: ev.VehicleId || "Unknown",
...
function listenToPanicEvents(clientId, callback) {
  const q2 = query(
    collection(db, "panicEvents"),
...
async function getAllPanicEvents(clientId) {
  try {
    const q2 = query(
      collection(db, "panicEvents"),
      where("clientId", "==", clientId)
